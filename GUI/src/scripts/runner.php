...
require_once "runner.marcell.php";
require_once "bg/marcell.php";
require_once "hr/marcell.php";
require_once "hu/marcell.php";
require_once "pl/marcell.php";
require_once "ro/marcell.php";
require_once "sk/marcell.php";
require_once "si/marcell.php";
...
while (($file = readdir($dh)) !== false) {
    ...
    if($corpus->loadData()){            
	$taskFile=$corpus->getFolderPath()."/tasks/old/".$tdata['task'];
	if(!is_file($taskFile)){
	    echo "tasFile not found [$taskFile]\n";
	    continue;
	}
	
        $taskDesc=json_decode(file_get_contents($taskFile),true);
...
function runTask($data){
        ...
        if($line!==false){
             foreach(explode(",",$current['columns']) as $col){
                  ...
                  if($taskDesc['type']=='basic_tagging'){
                      runMarcell($text,"${current['line']}_${col}_$fnameOut");
    ...
    /***** TEXT *****/
        ...
        if($taskDesc['type']=='basic_tagging'){
            runMarcell(file_get_contents($data['fpath']),$fnameOut);
        }else if($taskDesc['type']=='statistics'){
