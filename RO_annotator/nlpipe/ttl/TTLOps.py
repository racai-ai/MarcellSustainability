...
def startTTLProcess(port):
    ...
    os.execlp("perl", "perl", "PipTTL.pl", str(port))

# Author: Radu Ion (radu@racai.ro)
class TTLOps(PipApi):
    """This is the first non-Python NLP app to be included in the
    NLPipe platform. It's the TTL Perl module, adapted for NLPipe."""
    ...
    def __init__(self):
        super().__init__()
        self._algoName = PipAlgo.algoTTL
    ...
    def createApp(self):
        ...
        Process(target = startTTLProcess, args = [self._ttlPort], daemon = True).start()
